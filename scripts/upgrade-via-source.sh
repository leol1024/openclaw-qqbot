...
for APP_NAME in openclaw clawdbot moltbot; do
    ...
    if [ -f "$CONFIG_FILE" ]; then
        ...
        " 2>/dev/null || true)
        # 同时备份 streamSupport 配置
        if [ -z "$SAVED_STREAM_SUPPORT" ]; then
            SAVED_STREAM_SUPPORT=$(node -e "
                const cfg = JSON.parse(require('fs').readFileSync('$CONFIG_FILE', 'utf8'));
                const keys = ['qqbot', 'openclaw-qqbot', 'openclaw-qq'];
                for (const key of keys) {
                    const ch = cfg.channels && cfg.channels[key];
...
if [ -z "$SAVED_QQBOT_TOKEN" ] && [ -d "$HOME/.openclaw" ]; then
    ...
    if [ -z "$SAVED_STREAM_SUPPORT" ]; then
        ...
        if [ -n "$SAVED_STREAM_SUPPORT" ]; then
...
if [ -n "$STREAM" ]; then
    ...
    if [ "$CURRENT_STREAM_VALUE" = "$STREAM_VALUE" ]; then
        echo "✅ stream 配置已是目标值，跳过写入（避免配置覆盖提示）"
    elif openclaw config set channels.qqbot.streamSupport "$STREAM_VALUE" 2>&1; then
        echo "✅ stream配置成功"
        _config_changed=1
    else
        echo "⚠️  openclaw config set 失败，尝试直接编辑配置文件..."
        ...
        if [ -f "$OPENCLAW_CONFIG" ] && node -e "
          ...
          fs.writeFileSync('$OPENCLAW_CONFIG', JSON.stringify(cfg, null, 4) + '\n');
        " 2>&1; then
            echo "✅ stream配置成功（直接编辑配置文件）"
            _config_changed=1
        else
            echo "⚠️  stream配置设置失败，不影响后续运行"
        fi
    fi
else
