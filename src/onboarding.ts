...
"QQ Bot 配置",
    ...
    // 应用配置（markdownSupport 默认开启，streamSupport 默认开启）
    if (appId && clientSecret) {
      ...
      // 保留已有的 streamSupport 设置，新装默认 true
      const streamSupport = existingQQBot.streamSupport ?? true;
      ...
      if (accountId === DEFAULT_ACCOUNT_ID) {
        ...
        const acctStream = existingAccount.streamSupport ?? true;
